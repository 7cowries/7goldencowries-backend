// server.js — 7 Golden Cowries (LIVE; no dependency on subscriptions.active)
...
// ─────────────────────────────────────────────────────────────
// Preflight: create base tables (idempotent). We DO NOT rely on 'active' later.
const DB_FILE =
...
await predb.exec(`
  CREATE TABLE IF NOT EXISTS subscriptions (
    ...
    active INTEGER NOT NULL DEFAULT 0, /* present for new DBs, but we never read it */
    provider TEXT,
    tx_id    TEXT,
...
// seed quests once
const qcount = await predb.get(`SELECT COUNT(*) AS c FROM quests;`);
if (!qcount?.c) {
  const Q = [
...
// Prefer sqlite session store if available
let store;
try {
...
} catch { console.warn("[session] MemoryStore in use (install connect-sqlite3 for persistence)"); }

const SESSION_NAME = "7gc.sid";
app.use(session({
  name: SESSION_NAME,
  secret: process.env.SESSION_SECRET || "change-me",
  store, resave:false, saveUninitialized:false, rolling:true,
...
// materialize user if we can
app.use(async (req,_res,next)=>{
  try{
    if (req.session?.userId) return next();
    const hint = reqAddr(req);
    if (!hint) return next();
...
app.post("/api/auth/wallet/session", async (req,res)=>{
  ...
  req.session.userId = user.id; req.session.address = user.wallet;
...
// subscriptions: never read 'active' column; compute from tier
...
async function getSubStatus(wallet){
  ...
  return { active: tier !== "Free", tier, xpBoost: boostForTier(tier) };
}
app.get("/api/subscriptions/status", async (req,res)=>{
  const wallet = req.session?.address || reqAddr(req);
...
const priceForTier = (tier)=>{
  const m = { Explorer:Number(process.env.TON_PRICE_EXPLORER||0), Gold:Number(process.env.TON_PRICE_GOLD||0) };
  return m[tier] || m.Explorer || 0;
};
...
app.get("/api/v1/payments/ton/invoice/:id", async (req,res)=>{
  ...
  // Insert WITHOUT reading or requiring 'active'
  await db.run(
...
app.get("/api/v1/payments/status", async (req,res)=>{
  const wallet = req.session?.address || reqAddr(req) || null;
  const s = await getSubStatus(wallet);
