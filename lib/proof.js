...
export function proofToken(wallet, questId) {
  const secret = process.env.PROOF_SECRET || (process.env.NODE_ENV === 'test' ? '' : null);
  if (secret === null) throw new Error('PROOF_SECRET not set');
  const raw = `${wallet}:${questId}:${secret}`;
