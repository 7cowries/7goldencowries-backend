...
function extractSource(msg) {
  if (!msg || typeof msg !== 'object') return '';
  if (msg.source?.address) return normalizeAddress(msg.source.address);
  if (msg.source) return normalizeAddress(msg.source);
  if (msg.src?.address) return normalizeAddress(msg.src.address);
  if (msg.src) return normalizeAddress(msg.src);
  if (msg.from?.address) return normalizeAddress(msg.from.address);
  if (msg.from) return normalizeAddress(msg.from);
  if (msg.sender?.address) return normalizeAddress(msg.sender.address);
  if (msg.sender) return normalizeAddress(msg.sender);
  return '';
}
...
export async function verifyTonPayment({ txHash, to, from, minAmount = 0, comment }) {
  if (!txHash) {
  ...
  const source = extractSource(inbound);

  if (to && normalizeAddress(to) !== destination) {
    return { verified: false, reason: 'destination_mismatch', amount: amountTon, to: destination, from: source, comment: observedComment };
  }

  const normalizedFrom = normalizeAddress(from);
  if (normalizedFrom) {
    if (!source) {
      return { verified: false, reason: 'source_missing', amount: amountTon, to: destination, from: source, comment: observedComment };
    }
    if (normalizedFrom !== source) {
      return { verified: false, reason: 'source_mismatch', amount: amountTon, to: destination, from: source, comment: observedComment };
    }
  }

  if (minAmount && amountTon < Number(minAmount)) {
    return { verified: false, reason: 'amount_too_low', amount: amountTon, to: destination, from: source, comment: observedComment };
  }

  if (comment && !observedComment.includes(comment)) {
    return { verified: false, reason: 'comment_mismatch', amount: amountTon, to: destination, from: source, comment: observedComment };
  }
