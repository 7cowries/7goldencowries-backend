...
const initDB = async () => {
  const DEFAULT_PATH = "/var/data/7gc.sqlite3";
  const DB_FILE =
    process.env.SQLITE_FILE || process.env.DATABASE_URL || DEFAULT_PATH;
  ...
  await db.exec(`
    ...
    */
    ...
    -- Legacy proofs table used by questRoutes (kept for backward compatibility)
  ...
  // --- Crowns Arena / Payments / Sponsors ---
  await db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS arena_participants (
      ...
      updated_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY (arena_id) REFERENCES arenas(id),
      FOREIGN KEY (join_payment_id) REFERENCES payments(id)
    );
    ...
    CREATE TABLE IF NOT EXISTS sponsor_campaigns (
      ...
      updated_at TEXT DEFAULT (datetime('now')),
      FOREIGN KEY (sponsor_id) REFERENCES sponsors(id),
      FOREIGN KEY (sponsor_application_id) REFERENCES sponsor_applications(id),
      FOREIGN KEY (arena_id) REFERENCES arenas(id),
  ...
  // referrals
  await addColumnIfMissing("referrals", "completed", "INTEGER NOT NULL DEFAULT 0");
  await db.exec("UPDATE referrals SET completed = COALESCE(completed, 0)");

  // --- Backward-compatible column migrations (safe) ---
  // quests
  await addColumnIfMissing("quests", "code", "code TEXT");
  ...
  await ensureUniqueIndex("uq_quests_code", "ON quests(code)");
  ...
  await ensureUniqueIndex("uq_proofs_wallet_quest_url", "ON quest_proofs(wallet, quest_id, url)");
  await ensureUniqueIndex("uq_proofs_wallet_quest", "ON proofs(wallet, quest_id)");
  ...
  await ensureUniqueIndex("uq_referrals_code", "ON referrals(code)");
  ...
  await ensureUniqueIndex("uq_arena_participant", "ON arena_participants(arena_id, user_wallet)");
  await ensureUniqueIndex("uq_arena_claim", "ON arena_claims(arena_id, user_wallet, quest_id)");
  await ensureUniqueIndex("uq_payments_provider_ref", "ON payments(provider, provider_reference)");
  await ensureIndex("idx_arena_status_visibility", "ON arenas(status, visibility)");
  await ensureIndex("idx_arena_participants_arena_xp", "ON arena_participants(arena_id, arena_xp DESC)");
  await ensureIndex("idx_arena_quests_arena", "ON arena_quests(arena_id, active)");
  await ensureIndex("idx_payments_wallet_status", "ON payments(user_wallet, status)");
  await ensureIndex("idx_payment_events_provider_ref", "ON payment_events(provider, external_reference)");
  await ensureIndex("idx_reward_rules_arena", "ON reward_rules(arena_id)");
  await ensureIndex("idx_reward_payouts_arena", "ON reward_payouts(arena_id)");
  await ensureIndex("idx_sponsor_apps_status", "ON sponsor_applications(status)");
  await ensureIndex("idx_sponsor_campaigns_status", "ON sponsor_campaigns(status)");

  // legacy proofs table (used by questRoutes)
  await addColumnIfMissing("proofs", "reason", "TEXT");
  await addColumnIfMissing("proofs", "tweet_id", "TEXT");
  await addColumnIfMissing("proofs", "handle", "TEXT");
  ...
  await addColumnIfMissing("referrals", "code", `TEXT`);
  await db.exec("UPDATE referrals SET code = NULL WHERE code IS NULL OR code = ''");
  ...
  await addColumnIfMissing("subscriptions", "ton_amount",    `REAL DEFAULT 0`);
  await addColumnIfMissing("subscriptions", "tx_hash",      `TEXT`);
  await addColumnIfMissing("subscriptions", "tx_id",        `TEXT`);
  await addColumnIfMissing("subscriptions", "created_at",   `TEXT`);
  await addColumnIfMissing("subscriptions", "updated_at",   `TEXT`);
  ...
  await db.exec(
    `
      ...
      SET ton_amount = COALESCE(ton_amount, tonAmount, 0),
          tonAmount  = COALESCE(tonAmount, ton_amount, 0),
          created_at = COALESCE(created_at, timestamp, datetime('now')),
          updated_at = COALESCE(updated_at, timestamp, datetime('now'))
    `
  );
