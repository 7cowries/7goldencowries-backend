...
const initDB = async () => {
  ...
  await db.exec(`
    ...
    UPDATE users SET level=${BASE_LEVEL.level},         updatedAt=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE level IS NULL;
    ...
    UPDATE users SET nextXP=${BASE_LEVEL.nextNeed},          updatedAt=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE nextXP IS NULL;
