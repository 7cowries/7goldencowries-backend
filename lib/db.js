...
async function addColumnIfMissing(table, column, type) {
  ...
  if (!has) {
    ...
    // SQLite cannot add a NOT NULL column without a default, so if no
    // DEFAULT is present drop NOT NULL to keep the migration compatible.
    const hasDefault = /\bDEFAULT\b/i.test(t);
    if (!hasDefault) {
      t = t.replace(/\bNOT\s+NULL\b/gi, "").trim();
    }
    t = t.trim();
