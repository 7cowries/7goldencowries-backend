...
function adaptSql(sql) {
  ...
  // Drop SQLite pragmas that are invalid in Postgres.
  if (/^PRAGMA\s+/i.test(out)) return "-- noop pragma";

  // SQLite -> Postgres compatibility transforms used throughout routes.
...
const db = { run, get, all, exec, query: (text, values) => pool.query(text, values), pool };
export default db;
