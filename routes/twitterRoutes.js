...
const VERIFY_TARGET_ID = "1749440852192760064"; // @7goldencowries Twitter user ID
const ANNOUNCEMENT_TWEET_ID = "1945021057003401354"; // ✅ Actual pinned tweet ID

// Map wallet to twitter handle from DB
...
router.get("/verify/retweet/:wallet", async (req, res) => {
  ...
  try {
    ...
    const history = await fetch(`https://api.twitter.com/2/users/${userId}/tweets?tweet.fields=referenced_tweets`, {
      headers: { Authorization: `Bearer ${BEARER}` }
    });
