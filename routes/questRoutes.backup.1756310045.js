...
// Promisified helpers already provided by your db wrapper (await db.get/all/run)

// Derive quest fields safely from any schema version
...
// Discord: check if OAuth user token is in our guild
...
router.post("/complete", async (req, res) => {
  try {
    ...
    // One generic select works across schemas
    ...
    switch (quest.requirement) {
      case "x_follow":
        ...
        // If/when you add real follow checks, do them here using saved Twitter OAuth tokens.
        break;
      ...
      case "tg_bot_linked": {
        ...
        // (Optional) Add a 'tg_started' flag in users to verify they pressed /start on the bot.
        break;
      }
    ...
    // XP with multipliers
    const xpGain = Math.max(0, Math.round(quest.xp * (multiplierByTier[userTier] ?? 1)));
    ...
    // Referral bonus on first completion
